...
원칙:
- 반드시 1~2문장으로 완성 (활동이 많으면 2문장으로 자연스럽게 나눔)
- 모든 활동명은 반드시 포함하되 문장 속에 자연스럽게 녹아들도록 표현
- ~하였으며, ~하고, ~를 통해, ~수업에서는 등을 다양하게 활용해 활동 간 연결
- 활동명 뒤 표현을 다양하게 (활동에서 / 를 통해 / 수업에서는 / 시간에 등 반복 금지)
- 핵심 역량과 교육적 의미만 담고 불필요한 수식어 제거
- 어색하거나 원본을 그대로 옮긴 표현 사용 금지
- 서술어는 반드시 과거형 (~했습니다. ~보였습니다. ~발휘했습니다. 등)
- 개조식 문장(~임. ~함. ~보임.) 절대 사용 금지
- 각 결과 사이에 구분선(---, ===, ***) 절대 사용 금지
- 각 결과 앞에 반드시 ###${num} 형식으로 구분해주세요 (슬래시(/) 사용 금지, 반드시 원본 번호 그대로)

예시)
원본: "준비 운동 만들기 활동에서 루틴 설계함. 스포츠 생각 꺼내기 활동에서 참여함. 축구 기초 기능에서 피드백 제시함."
변환: "준비 운동 만들기 활동에서 수업 목표에 맞는 루틴을 체계적으로 설계하였으며, 스포츠 안에서 생각과 관점 꺼내기를 통해 논리적 사고를 꾸준히 발전시켰습니다. 축구 기초 기능 수업에서는 팀원의 동작을 세밀하게 분석하고 구체적인 피드백을 제시하며 협력적으로 기술 향상에 기여하는 탁월한 동료 지원 능력을 발휘했습니다."
...
function convertEnding(text){
  ...
  const specific=[
    ...
    ['이 나타남\\.','이 나타났습니다.'],['가 나타남\\.','가 나타났습니다.'],
  ];
  specific.forEach(([p,r])=>{result=result.replace(new RegExp(p,'g'),r);});
  // 2단계: 이미 과거형
  result=result.replace(/했음\./g,'했습니다.').replace(/됐음\./g,'됐습니다.').replace(/였음\./g,'였습니다.');
  // 3단계: 포괄적 어미 변환
  result=result.replace(/([가-힣]+)함\./g,(m,p1)=>p1+'했습니다.');
  result=result.replace(/([가-힣]+)임\./g,(m,p1)=>p1+'이었습니다.');
  result=result.replace(/([가-힣]+)됨\./g,(m,p1)=>p1+'되었습니다.');
  // 4단계: 남은 특수 어미
  result=result.replace(/보임\./g,'보였습니다.').replace(/않음\./g,'않았습니다.');
  result=result.replace(/없음\./g,'없었습니다.').replace(/있음\./g,'있었습니다.');
  result=result.replace(/즐김\./g,'즐겼습니다.').replace(/나타남\./g,'나타났습니다.');
  result=result.replace(/드러남\./g,'드러났습니다.').replace(/이어짐\./g,'이어졌습니다.');
  result=result.replace(/([가-힣]+)남\./g,(m,p1)=>p1+'났습니다.');
  result=result.replace(/([가-힣]+)음\./g,(m,p1)=>p1+'았습니다.');
  return result;
}
